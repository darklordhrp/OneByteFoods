...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($selected_tables as $table_id) {
        ...
        if (!$stmt->execute()) {
            die("Error updating table availability: " . $stmt->error);
        }
    }
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        if ($result_user->num_rows > 0) {
            ...
            // Insert booking into booked_tables_two for each selected table
            foreach ($selected_tables as $table_id) {
                $sql_booking = "INSERT INTO booked_tables_two (table_number, booking_date, booking_time, user_name, user_email) 
                VALUES (?, ?, ?, ?, ?)";
...
<body>
    ...
    function reserveTable() {
        ...
        var params = 'date=' + encodeURIComponent(date) + '&time=' + encodeURIComponent(time) + '&seats=' + encodeURIComponent(selectedTables.length * 2) + '&cost=' + encodeURIComponent(selectedTables.length * 200) + '&selected_tables=' + encodeURIComponent(JSON.stringify(selectedTables));
        xhr.send(params);
    }
